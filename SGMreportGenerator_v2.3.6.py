...
def get_PatientInfo_fromPyfile(selected_folder, self):
    ...
    if python_files:
        try:
            for StaticDataFileName in python_files:   # python_files.reverse()
                print(StaticDataFileName)
                # define python file name
                # StaticDataFileName = os.path.join(selected_folder, py_file)
        ...
        # ----------------------------- Plot data -----------------------------
            ...
            for Lnum in plotloop:
                ...
                if gcd_count == 0:
                    ...
                    filenamestr = ('Visit')
                    plt.gcf().text(firstcol, titleRow, filenamestr, fontsize=mf, color='k')
                    plt.gcf().text(nextcol, titleRow, filenamestr, fontsize=mf, color='k')
                    filenamestr = ('Cond')
                    plt.gcf().text(firstcol+0.04, titleRow, filenamestr, fontsize=mf, color='k')
                    plt.gcf().text(nextcol+0.04, titleRow, filenamestr, fontsize=mf, color='k')
                    
                    filenamestr = ('File')
                    plt.gcf().text(firstcol+0.09, titleRow, 'Left-'+filenamestr, fontsize=mf, color='k')
                    plt.gcf().text(nextcol+0.09, titleRow, 'Right-'+filenamestr, fontsize=mf, color='k')
                    ...
                    GDM = 100 # round(dataNmean_dict['GDI'][0],2)
                ...
                # Right
                    ...
                    try:
                        ...
                        plt.gcf().text(nextcol+0.185, rowvec[LeftPlotNum-1], RGD, fontsize=sf-1, color='k')
                    except:
                ...
                if Lnum == 0 and LeftPlotNum < 7:
                    # Add date from GCD filename - ONLY ON LEFT
                    plt.gcf().text(firstcol-0.08, rowvec[LeftPlotNum-1], gcdfilestr[8:18], fontsize=sf, color=cc[PlotNum])
                    # Add visit letter from GCD filename
                    plt.gcf().text(firstcol+0.01, rowvec[LeftPlotNum-1], gcdfilestr[0], fontsize=sf, color=cc[PlotNum])
                    # Add condition from GCD filename                
                    plt.gcf().text(firstcol+0.05, rowvec[LeftPlotNum-1], gcdfilestr[1:3], fontsize=sf, color=cc[PlotNum])
                    # Add filename
                    plt.gcf().text(firstcol+0.09, rowvec[LeftPlotNum-1], gcdfilestr[0:7] +'...' +gcdfilestr[-6:-4], fontsize=sf, color=cc[PlotNum])
                ...
                if (Lnum == 1 and RightPlotNum < 6 and gcdNum_selected >= 12) or (Lnum == 1 and RightPlotNum < 7 and gcdNum_selected < 12):
                    # Add visit letter from GCD filename
                    plt.gcf().text(nextcol+0.01, rowvec[LeftPlotNum-1], gcdfilestr[0], fontsize=sf, color=cc[PlotNum])
                    # Add condition from GCD filename                
                    plt.gcf().text(nextcol+0.05, rowvec[LeftPlotNum-1], gcdfilestr[1:3], fontsize=sf, color=cc[PlotNum])
                    # ADd filename
                    plt.gcf().text(nextcol+0.09, rowvec[RightPlotNum-1], gcdfilestr[0:7] +'...' +gcdfilestr[-6:-4], fontsize=sf, color=cc[PlotNum])
    ...
    for file in checkboxes:
        ...
        # ----------------------------- Plot data -----------------------------
            ...
            for Lnum in plotloop:
                ...
                try: 
                    ax.plot(x,data_dict[limb_spec[Lnum] + data_label[0]], color=cc[PlotNum], linewidth=0.75)  
                    ax.set_title('Hip Extension Moment')
                    ax.set_ylabel('Nm/kg')
                    ax.set_xlim([0,datLen])
                    ax.set_xticks(list(range(0,datLen,xts)), ['','','','','',''])
                    # y-limits
                ...
                try:
                    ax.plot(x,data_dict[limb_spec[Lnum] + data_label[0]], color=cc[PlotNum], linewidth=0.75)
                    ax.set_title('Knee Extension Moment')
                    ax.set_xlim([0,datLen])
                    ax.set_xticks(list(range(0,datLen,xts)), ['','','','','',''])
                    # y-limits
                    ylower = min(min(data_dict[limb_spec[Lnum] + data_label[0]]),-1)
                ...
                try:
                    ax.plot(x,data_dict[limb_spec[Lnum] + data_label[0]], color=cc[PlotNum], linewidth=0.75)
                    ax.set_title('Ankle Plantarflexion Moment')
                    ax.set_xlim([0,datLen])
                    ax.set_xticks(list(range(0,datLen,xts)), ['','','','','',''])
                    # y-limits
                    ylower = min(min(data_dict[limb_spec[Lnum] + data_label[0]]),-1)
